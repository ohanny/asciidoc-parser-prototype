...
public class CalloutProcessor {
    ...
    private ListingLineBuilder context;

    public static CalloutProcessor newInstance() {
        return new CalloutProcessor();
    }

    public void processCallouts(ListingLineBuilder context) {
        this.context = context;
        this.position = context.offset + context.length - 1;

        while (callout()) {
            context.length = position + 1 - context.offset;
            context.addCallout(0, Callout.of(getCalloutNumber(), null));
            //context.addCallout(0, ListingCallout.of(getCalloutNumber(), context.lineNumber)); TODO
        }
    }
