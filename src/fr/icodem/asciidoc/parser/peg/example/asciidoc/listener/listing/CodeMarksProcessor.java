...
public class CodeMarksProcessor {
    ...
    private void processLine(LineContext line, List<HighlightParameter> params) {
        List<HighlightParameter> paramsForLine = filterAndSort(params, line);
        buildChunks(line, paramsForLine);
    }

    private List<HighlightParameter> filterAndSort(List<HighlightParameter> params, LineContext line) {
    ...
    private HighlightParameter fixEdgesOfLine(LineContext line, HighlightParameter par) {
    ...
    private void buildChunks(LineContext line, List<HighlightParameter> params) {
        List<LineChunkContext> chunks = getChunks(1, line.length, line.lineNumber,
                                                  line.data, line.offset, params);

        if (chunks == null || chunks.isEmpty()) {
            chunks = Collections.singletonList(LineChunkContext.of(line.data, line.offset, line.length, 1, line.length));
        }
    ...
    private void buildChunks(LineChunkContext chunk, LineContext line, List<HighlightParameter> parameters) {
    ...
    private List<HighlightParameter> paramsForChunk(List<HighlightParameter> params, LineChunkContext chunk) {
        return params.stream()
                     .filter(p -> isParamForChunk(p, chunk))
                     .collect(Collectors.toList());
    }

    private boolean isParamForChunk(HighlightParameter param, LineChunkContext chunk) {
        return  !param.isProcessed() && param.getFrom().getColumn() >= chunk.columnFrom && param.getTo().getColumn() <= chunk.columnTo;
    }

    private List<LineChunkContext> getChunks(int columnFrom, int columnTo, int lineNumber, char[] data,
        ...
        for (int pos = columnFrom; pos <= columnTo;) {
            ...
            LineChunkContext chunk = LineChunkContext.of(data, offset + pos - 1, length, pos, pos + length - 1);
